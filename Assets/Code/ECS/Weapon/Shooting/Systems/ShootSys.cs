...
namespace ECS.Weapon.Shooting {
   public class ShootSys : IEcsRunSystem, IEcsInitSystem {
      ...
      public void Init(IEcsSystems systems) {
         ...
         _filter = _world.Filter<WeaponTag>().Inc<Projectiles>().Inc<ProjectilesSpawnOrigins>().Inc<WantAttack>().Exc<BlockAttack>().End();
      ...
      public void Run(IEcsSystems systems) {
         foreach (int weaponE in _filter) {
            foreach (EcsTransform shootOrigin in ShootOrigins(weaponE)) {
               ...
               SpawnRandomProjectile(WithSpread(weaponE, ref insTransform), weaponE);
            }
